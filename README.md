...
`mcp-console serve` communicates with its MCP client over standard input and output.
On macOS, the server starts one `mcp-console sandbox` launcher for each worker generation, with the built-in or configured relay command line as its target.
The launcher is the sole host-side sandbox owner.
It retains the sandbox root and manager waitably, holds the manager ownership token, and releases the relay's startup gate only after the manager installs observation, adopts the private directory, and can be recovered if it fails.
The server owns only the launcher's standard input and output plus normal child exit, signaling, and reaping; the launcher passes those streams through to the relay and writes infrastructure diagnostics only to standard error.
The server first requests graceful shutdown through the relay protocol and sends `SIGTERM` to the launcher if the relay misses its deadline.
In the hidden parent-owned mode used by the server, that signal or server exit requests managed retirement.
When managed cleanup succeeds, the launcher exits after manager cleanup and root reaping, so that normal exit is the server's cleanup barrier.
If the launcher itself is killed or crashes, ownership-token EOF still asks the manager to clean up, but the server can no longer wait synchronously for that manager.
The manager retires descendants it observed, including descendants that entered another process group or session, and removes the private directory only after successful cleanup.
Unexpected manager loss triggers bounded launcher-side recovery while the direct root remains live and waitable; that fallback performs process cleanup only and cannot recover a descendant that had already detached from the root's ancestry.
A later descendant that becomes orphaned before the manager resolves its fork event remains outside the cleanup guarantee.

The same `sandbox` command is available directly for development.
Its requested command remains blocked on a private startup gate until the manager reports readiness and the launcher installs manager-failure recovery.
The command inherits standard input, output, and error while unrelated inherited file descriptors are closed before the target runs.
