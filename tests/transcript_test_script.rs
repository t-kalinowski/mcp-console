...
#[cfg(unix)]
#[test]
fn transcript_test_script_ignores_suites_in_private_directories() {
    let repository = Path::new(env!("CARGO_MANIFEST_DIR"));
    let (temporary, transcripts, _release_gate) = runner_fixture("private-directories", "30.0");
    let nested_suite = transcripts.join("client_server/nested/suite.py");
    let private_suite = transcripts.join("client_server/_private/suite.py");
    let nested_golden = transcripts.join("golden/client_server/nested/suite/is_discovered.yaml");
    for parent in [
        nested_suite.parent().unwrap(),
        private_suite.parent().unwrap(),
        nested_golden.parent().unwrap(),
    ] {
        fs::create_dir_all(parent).unwrap();
    }
    fs::copy(
        repository.join("tests/fixtures/transcript_runner/nested.py"),
        &nested_suite,
    )
    .unwrap();
    fs::copy(
        repository.join("tests/fixtures/transcript_runner/private.py"),
        &private_suite,
    )
    .unwrap();
    fs::copy(
        repository.join("tests/fixtures/transcript_runner/nested.yaml"),
        nested_golden,
    )
    .unwrap();

    let nested_executed = temporary.path().join("nested-executed");
    let private_loaded = temporary.path().join("private-loaded");
    let private_executed = temporary.path().join("private-executed");
    let output = Command::new("uv")
        .args(["run", "--script"])
        .arg(transcripts.join("_run.py"))
        .arg("--list")
        .current_dir(temporary.path())
        .env("NESTED_SUITE_EXECUTED", &nested_executed)
        .env("PRIVATE_SUITE_LOADED", &private_loaded)
        .env("PRIVATE_SUITE_EXECUTED", &private_executed)
        .output()
        .unwrap();
    assert!(
        output.status.success(),
        "listing transcript tests failed: {}",
        String::from_utf8_lossy(&output.stderr)
    );
    let listed = String::from_utf8(output.stdout).unwrap();
    assert!(
        listed.contains("client_server/nested/suite::is_discovered\n"),
        "ordinary nested suite was not listed: {listed:?}"
    );
    assert!(
        !listed.contains("client_server/_private/suite"),
        "private suite was listed: {listed:?}"
    );
    assert!(!private_loaded.exists(), "private suite was loaded");
    assert!(
        !private_executed.exists(),
        "private suite case was executed"
    );

    let output = Command::new("uv")
        .args(["run", "--script"])
        .arg(transcripts.join("_run.py"))
        .arg("client_server/nested/suite::is_discovered")
        .current_dir(temporary.path())
        .env("NESTED_SUITE_EXECUTED", &nested_executed)
        .env("PRIVATE_SUITE_LOADED", &private_loaded)
        .env("PRIVATE_SUITE_EXECUTED", &private_executed)
    ...
    assert!(
        output.status.success(),
        "ordinary nested suite failed: {}",
        String::from_utf8_lossy(&output.stderr)
    );
    assert!(
        nested_executed.exists(),
        "ordinary nested suite did not run"
    );
    assert!(!private_loaded.exists(), "private suite was loaded");
    assert!(
        !private_executed.exists(),
