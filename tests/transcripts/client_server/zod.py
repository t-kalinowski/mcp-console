...
def test_shutdown_retries_reap_after_manager_stop_failure(
    binary: Path,
) -> Transcript:
    zod = Path(__file__).resolve().parents[2] / "fixtures" / "zod"
    with tempfile.TemporaryDirectory() as temporary_directory:
        temporary = Path(temporary_directory)
        manager_stop_failed = FifoCheckpoint(temporary / "manager-stop-failed")
        relay_reap_failed = FifoCheckpoint(temporary / "relay-reap-failed")
        relay_reap_retried = FifoCheckpoint(temporary / "relay-reap-retried")
        environment = os.environ.copy()
        environment["TMPDIR"] = temporary_directory
        environment["ZOD_REPORT_PROCESS_GROUP"] = "1"
        environment["MCP_CONSOLE_TEST_MANAGER_STOP_SEND_FAILURE"] = str(
            manager_stop_failed.path
        )
        environment["MCP_CONSOLE_TEST_RELAY_REAP_FAILURE"] = str(relay_reap_failed.path)
        environment["MCP_CONSOLE_TEST_RELAY_REAP_RETRY"] = str(relay_reap_retried.path)
        ...
        try:
            ...
            worker_group = read_worker_group(
                wait_for_marker(temporary, "zod-process-group", client)
            )
            assert relay_target == worker_group, (
                "helper did not stop the sandbox process-group leader"
            )
            ...
            wait_for_stopped_process(
                ...
                worker_group,
                client,
                "relay shutdown reap retry",
            )

            # EOF shutdown owns the child before and after joining relay I/O.
            # The second pass must retry if the first one could not reap it.
            client.stdin.close()
            manager_stop_failed.wait("manager STOP send failure")
            relay_reap_failed.wait("first relay reap failure")
            relay_reap_retried.wait("relay reap retry")
            return_code = client.process.wait(timeout=10)
            extra_output = client.stdout.read()
            standard_error = client.stderr.read()

            assert return_code == 1, standard_error
            assert extra_output == "", f"unexpected extra output: {extra_output}"
            assert not process_group_exists(worker_group), (
                "stopped relay process group outlived server shutdown"
            )
            assert not process_exists(relay_target), "server did not reap the relay"
            assert not process_exists(helper_pid), (
                "detached worker descendant outlived server shutdown"
            )
            client.transcript.append(
                {
                    "server_returncode": return_code,
                    "server_stderr": standard_error,
                }
            )
            passed = True
            return client.transcript
            ...
            if not passed:
                ...
                stop_process_group(worker_group)
                stop_process(client.process)
            manager_stop_failed.close()
            relay_reap_failed.close()
            relay_reap_retried.close()
