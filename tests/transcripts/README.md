...
The `worker` suite records the built-in worker's parsed sideband messages and observed standard-stream chunks as YAML mappings in approximate event order.
It drains standard output and standard error immediately before recording each sideband message, but does not define ordering between the two standard streams.
