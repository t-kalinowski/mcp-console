...
For observable platform differences, its explicit `platform="linux"` or `platform="darwin"` selects `CASE.PLATFORM.yaml`.
There is no automatic platform fallback; updates preserve the other platform's snapshots and any shared snapshot.
...
On macOS, that relay is the sandbox root and process-group leader behind the server's owned launcher child.
On Linux, the client harness passes `--no-sandbox`, and the server owns the relay directly.
Both use the same fd 0/1/2 boundary as the production relay.
...
The proxy starts the built-in worker inside the server's launcher-owned sandbox on macOS, or directly in `--no-sandbox` mode on Linux, forwards sideband messages and standard streams, and writes parsed events to its private temporary directory for the test to read before shutdown.
...
The suite also verifies both documents with Yamark, and the optional Quarto suite executes generated R and Python cells through `ir` (inside the standalone sandbox on macOS) when `ir` and `quarto` are installed.
...
- `events.py` provides blocking file-change and process-exit observation with kqueue on macOS and inotify/pidfds on Linux.
...
A mixed suite may set `CASE_PLATFORMS = {"case_name": {"darwin"}}` for individual cases, omitting the `test_` prefix.
Linux exercises the shared runtime and protocol suites with `--no-sandbox`; cases requiring isolation, sandbox-manager recovery, or descendant cleanup remain macOS-only.
...
The dependency-free Zod worker fixture uses `python3` directly so direct-worker signal and reaping tests observe the interpreter rather than a `uv` wrapper; the test environment requires Python 3.11 or later.
