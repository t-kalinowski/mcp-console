...
When evaluated code explicitly asks for input or enters a debugger, the session switches to an input state and accepts line-oriented `stdin`.
...
Explicit stdout and stderr are retained completely in session files, while each MCP reply contains only a bounded current excerpt.

### 8. Durable context outside the conversation

Each session maintains a generated `transcript.qmd` containing submitted code, labels, bounded output, errors, input interactions, and artifact paths.
...
New capabilities should usually appear as ordinary language code, runtime helpers, files, or private protocol extensions rather than additional globally visible MCP tools.
Every new public field or tool has a permanent model-context cost.
