...
Release and ordinary local builds use the same native GNU targets and companion build path on Linux; neither requires Zig or a musl toolchain.
...
Linux builds require a C compiler, binutils (`readelf` and `strip`), `pkg-config`, libcap development files, and OpenSSL development files (`build-essential binutils pkg-config libcap-dev libssl-dev` on Ubuntu).
The installed release helper requires `libcap.so.2`; end users do not need the compiler or development headers.
OpenSSL development files are a build prerequisite, but the audited native x86-64 executable files have no dynamic OpenSSL dependency.
Source builds use the host libraries and retain their corresponding runtime dependencies.

After building a Linux wheel, run `python3 scripts/release.py audit-linux-wheel dist/*.whl`.
This checks the filename and `WHEEL` platform tags, the architecture, loader, and shared-library dependencies of all three executables, and their required GLIBC symbol versions against the declared baseline.
Maturin's application audit alone does not inspect private executables in wheel data.
Wheel tags alone do not cover the helper's additional libcap requirement or libraries loaded later by R, Python, or SQL.
Linux smoke tests exercise the bundled helper with an empty `PATH`, check installed-layout integrity and supported host-helper selection, and evaluate R through default sandboxed `serve`.
See the [runtime validation record](docs/LINUX_COMPATIBILITY.md#runtime-portability-investigation) for the separate R/Python/SQL probes and their limitations.
