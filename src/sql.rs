    ...
    tryCatch(
      {
        ...
        cast_projection <- paste0(
          "CAST(preview.", identifiers, " AS VARCHAR) AS ", identifiers,
          collapse = ", "
        )
        value_projection <- paste0(
          "CASE WHEN length(strings.", identifiers, ") > ", cell_width,
          " THEN left(strings.", identifiers, ", ", cell_width - 1L,
          ") || '…' ELSE strings.", identifiers, " END AS ", identifiers
        )
        ...
        truncated_projection <- paste0(
          "coalesce(length(strings.", identifiers, ") > ", cell_width,
          ", false) AS ", truncated_identifiers
        )
        query <- paste(
