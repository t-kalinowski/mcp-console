...
pub(crate) fn kill(process_group_id: u32) -> io::Result<()> {
    ...
    kill_members(process_group_id, None)
}

pub(crate) fn kill_members_except(
    process_group_id: u32,
    excluded_process_id: u32,
) -> io::Result<()> {
    let process_group_id = valid_process_id(process_group_id, "process group")?;
    let excluded_process_id = valid_process_id(excluded_process_id, "excluded process")?;
    kill_members(process_group_id, Some(excluded_process_id))
}
