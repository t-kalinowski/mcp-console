...
struct SendArguments {
    /// Complete multiline R cell evaluated in persistent global state. The built-in R environment
    /// includes tidyverse and reticulate; packages are not attached automatically. Common entry
    /// points include `ggplot2::ggplot()`, `dplyr::mutate()`, `readr::read_csv()`, and
    /// `jsonlite::fromJSON()`. Read Python globals through `py$name`; for example,
