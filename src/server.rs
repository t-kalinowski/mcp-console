...
struct SendArguments {
    ...
    stdin: Option<String>,
    /// Omit `timeout_ms` for normal calls, including polls (default: 60,000 milliseconds). The call
    /// returns as soon as execution finishes; the timeout is a maximum wait, not a fixed delay.
    /// To wait, use the default or a longer timeout instead of repeated short polls. Use `timeout_ms: 0`
    /// to deliberately start background work. The wait starts after cell dispatch or attachment to an
    /// active evaluation and includes one automatic worker replacement attempt. Reaching the timeout
