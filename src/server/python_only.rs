    ...
    *description = description.replace("`r`, `python`, or `sql` cell", "`python` cell");
    ...
    if python_preparation {
        for (field, description) in [
            ...
            (
                "control",
                "Applies lifecycle control alone or before compatible same-call fields. interrupt requests SIGINT and preserves Python state; same-call stdin is queued before the interrupt grace. Python requirement additions with interrupt are rejected before signaling or queuing input. restart discards objects and unread stdin, then sends same-call stdin and code only to the replacement. With requirements.python, the complete candidate environment is resolved and inspected before stopping the current worker. Preparation failure preserves the current worker, retained requirements, and queued input; same-call code and stdin are not sent. Retirement or replacement failure follows the ordinary restart contract. Plain restart reuses the accepted environment.",
            ),
        ] {
            if let Some(property) = properties.get_mut(field) {
                property["description"] = description.into();
        ...
        fields.get_mut("python").expect("Python requirements")["description"] = "Named PEP 508 registry requirements added to the retained Python manifest. Local paths, URLs, editable requirements, and archives are rejected. Preparation uses the session's trusted uv configuration outside the worker sandbox and may execute package build code. Live changes require control: restart.".into();
    } else {
