...
pub(super) fn description(
    ...
    if let Some(target) = target {
        ...
        match kind {
            ...
            _ => description.push_str("Dependency capability is discovered there. When available, managed defaults and requested R, Python, and DuckDB dependencies are prepared outside the worker sandbox with the remote account's trusted setup permissions; bare runtimes require preinstalled packages. Records and returned images are saved on the controller beneath its existing project .agents/console directory or its home .agents/console directory. Files created by code remain remote. The source-only Quarto export does not reproduce the remote filesystem."),
        }
    }
    description
}
