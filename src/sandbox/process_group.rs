...
pub(super) fn kill(process_group_id: u32) -> io::Result<()> {
...
fn kill_members(process_group_id: libc::pid_t) -> io::Result<()> {
    // Group signalling and the EPERM fallback both operate on PID snapshots.
    // The caller normally keeps the direct leader unreaped while
    ...
    loop {
        let mut members = members(process_group_id)?;
        ...
        for process_id in members {
            ...
            match is_live_member(process_id, process_group_id) {
                Ok(false) => continue,
        ...
        std::thread::sleep(STOP_POLL_INTERVAL.min(deadline.saturating_duration_since(now)));
    }
}

fn members(process_group_id: libc::pid_t) -> io::Result<Vec<libc::pid_t>> {
...
fn is_live_member(process_id: libc::pid_t, process_group_id: libc::pid_t) -> io::Result<bool> {
...
fn group_of(process_id: libc::pid_t) -> io::Result<Option<libc::pid_t>> {
