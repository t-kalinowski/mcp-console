...
The private executable is pinned in `sandbox-runner.json` and installed under the prefix's `libexec` directory.
Console verifies its executable-relative path and build-bound digest before exec; Cargo and installed-wheel layouts use the same lookup.
