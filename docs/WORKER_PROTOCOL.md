...
For the built-in worker on macOS, server initialization first asks IR to resolve the retained default R requirements `tidyverse` and `github::rstudio/reticulate` outside the sandbox.
...
Before worker startup, the server merges exact strings with the retained tidyverse and GitHub reticulate requirements and managed Python baseline, then resolves the complete candidates outside the sandbox.
...
The default IR library supplies tidyverse, including dplyr, pillar, and tibble, plus the worker's GitHub reticulate build, without attaching packages automatically.
...
MCP Console does not automatically install the host-bootstrap reticulate or the remaining SQL runtime packages.
