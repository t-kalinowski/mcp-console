...
At Python cell end, the worker renders each open `matplotlib.pyplot` figure in memory, emits it once as `image/png`, and closes every pyplot-managed figure; `plt.show()` is not required.
