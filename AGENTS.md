...
Worker `output` frames carry `output` or `diagnostic` console channels; an omitted channel defaults to `output` for compatibility with existing custom workers.
...
The worker maps `R_WriteConsoleEx` type 0 to the `output` channel, nonzero types and `R_ShowMessage` to `diagnostic`, and currently renders both as ordinary MCP text.
...
Within the worker process, reticulate then routes Python standard output through R's `output` console channel and Python standard error, including tracebacks, through `diagnostic` sideband `output` frames.
